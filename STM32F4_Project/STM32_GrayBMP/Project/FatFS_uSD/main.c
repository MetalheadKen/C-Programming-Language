...
char read[256] ;
unsigned char image[LENGTH * WIDTH + HEADER]=  {0};
uint8_t color[4];
  ...
  */  
  ...
  f_close(&file);*/	
	...
	*/
	...
	while (1)
  {
		LCD_DrawMonoPict((const uint32_t *)image);
		Delay(100); //Delay 1 sec.
		
		//LCD_DrawMonoPict((const uint32_t *)photo7);
		LCD_DrawGrayPict((const uint8_t *)image);
		Delay(100);
		
		LCD_Draw16BitBMP((const uint16_t *)welcome);
		Delay(100);
		
		//LCD_WriteBMP((uint32_t)imagen);
